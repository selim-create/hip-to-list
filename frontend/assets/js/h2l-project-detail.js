(function(wp) {
    ...
    const { ListView, BoardView } = window.H2L.Tasks; // TaskDetailModal kaldırıldı (Global oldu)
    ...
    // GÜNCELLEME: onTaskClick prop olarak alındı
    window.H2L.ProjectDetail = ({ project, projects, folders, tasks, sections, users, navigate, onAddTask, onDeleteTask, onUpdateTask, onAddSection, onAction, onUpdateSection, onDeleteSection, labels, onTaskClick }) => {
        ...
        // GÜNCELLEME: selectedTask state'i kaldırıldı, modal global App üzerinden yönetiliyor.
                ...
                el('div', { className: 'h2l-detail-header' },
                    el('div', { className: 'h2l-header-top' },
                        ...
                        el('div', { className: 'h2l-header-actions' },
                            ...
                            !isPrivateFolder && el('div', { style: { position: 'relative' }, ref: shareMenuRef }, el('button', { className: 'h2l-action-btn', title: 'Üye Ekle / Çıkar', onClick: () => setIsShareMenuOpen(!isShareMenuOpen) }, el(Icon, { name: 'user-plus' }), 'Paylaş'), isShareMenuOpen && el(MemberSelectMenu, { users, project, onUpdateMembers: handleUpdateMembers })),
                            el('div', { style: { position: 'relative' }, ref: viewMenuRef }, el('button', { className: 'h2l-action-btn', onClick: () => setIsViewMenuOpen(!isViewMenuOpen) }, el(Icon, { name: 'sliders' }), 'Görünüm'), isViewMenuOpen && el('div', { className: 'h2l-popover-menu right-aligned' }, el('div', { className: 'h2l-menu-section' }, el('span', { className: 'h2l-menu-title' }, 'DÜZEN'), el('div', { className: 'h2l-view-selector' }, el('div', { className: `h2l-view-btn ${viewMode==='list'?'active':''}`, onClick: () => setViewMode('list') }, el(Icon, { name: 'list' }), ' Liste'), el('div', { className: `h2l-view-btn ${viewMode==='board'?'active':''}`, onClick: () => setViewMode('board') }, el(Icon, { name: 'table-columns' }), ' Pano'))), el('div', { className: 'h2l-menu-section' }, el('div', { className: 'h2l-switch-item', onClick:()=>setShowCompleted(!showCompleted) }, el('span', null, 'Tamamlanan görevler'), el('div', { className: `h2l-toggle-switch ${showCompleted?'on':''}` }, el('div', {className:'knob'})))))),
                            el('div', { style: { position: 'relative' }, ref: moreMenuRef }, el('button', { className: 'h2l-action-btn', title: 'Diğer', onClick: () => setIsMoreMenuOpen(!isMoreMenuOpen) }, el(Icon, { name: 'ellipsis' })), isMoreMenuOpen && el('div', { className: 'h2l-popover-menu right-aligned' }, el('div', { className: 'h2l-menu-item', onClick: () => { if(onAction) onAction('edit_project', project); setIsMoreMenuOpen(false); } }, el(Icon, { name: 'pen' }), ' Projeyi düzenle'), el('div', { className: 'h2l-menu-item' }, el(Icon, { name: 'box-archive' }), ' Projeyi arşivle'), el('div', { className: 'h2l-menu-item text-danger', onClick: () => { if(onAction) onAction('delete_project', project.id); setIsMoreMenuOpen(false); } }, el(Icon, { name: 'trash' }), ' Projeyi sil')))
                        )
                    )
                )
            ),
